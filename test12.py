...
#1337 biches.
#i do know that you can solve this easily with integer factorization. this is just for fun.
from sys import *
...
class crypt():
	...
	def encrypt(f):
		with open(f,'r') as result:
			...
			while True:
				...
				i+=1
		try:
			secretkey=getpass("input new key:")
			secretkey2=getpass("input key again:")
	...
	def decrypt(f):
		with open(f,'r') as stuff:
			...
			i=i.split(',')
		try:
			secretkey=getpass("input password:")
		except KeyboardInterrupt:
			exit()
		if not secretkey:
			secretkey="badasscryptography"
